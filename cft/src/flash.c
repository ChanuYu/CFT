...
struct ssd_info *process(struct ssd_info *ssd)   
{
    ...
    if(flag==1)
    {
        ...
        else if(check_idle_status(ssd)) //return (ssd->idle_flag > 0) ? SUCCESS : FAILURE;
        {
            ...
            calc_offprom_benefit(ssd);

            ////////////////////////////////////////////////////////////////////
            //if((plane_num = check_prom_condition(ssd,prev_prom_count)) != -1)
            //{
                //count = promote_to_slc(ssd,plane_num);
                //if(ssd->proc_time_used)
                // = 1;
            //}
            ////////////////////////////////////////////////////////////////////
...
int check_condition_for_comp(struct ssd_info *ssd)
{
    ...
    int min_slc_blks = ssd->parameter->block_plane * 0.01;
    int created_blocks = p_plane->slc_pool->num_blks;
    if(created_blocks < min_slc_blks)
        return SUCCESS;

    float alpha = ssd->parameter->alpha;
    float perform_gain = alpha * ((ssd->slc_r_trx/(float)ssd->total_r_trx) * ssd->r_perform_gain_latency + (ssd->slc_w_trx/(float)ssd->total_w_trx) * ssd->w_perform_gain_latency);
    float perform_cost = (ssd->decomp_read_count/(float)ssd->total_r_trx) * ssd->parameter->time_characteristics.tDecomp;
...
{
    if(!ssd->total_slc_blk)
        return;

    float slc_pages = ssd->total_slc_blk / (float)(ssd->parameter->page_block>>2);
    float avg_slc_access = ssd->slc_r_trx / slc_pages;
    
    if(ssd->r_perform_gain_latency * avg_slc_access > (ssd->parameter->time_characteristics.tPROG_slc / (float)1000))
        ssd->collect_offprom = 1;
    else
        ssd->collect_offprom = 0;
}

int check_exec_condition_for_promotion(struct ssd_info *ssd)
{
    return (ssd->off_prom_count > 0) ? TRUE : FALSE;
}
...
int exec_off_line_promotion(struct ssd_info *ssd)
{
    ...
    int search_count = 0;
    int search_threshold = ssd->parameter->chip_num / 2;
    int chip_count = 0;
    ...
    while(curr!=NULL && search_count < search_threshold)
    {
...
{
    return (ssd->used_blocks/(float)ssd->total_blocks) > 0.9 ? TRUE : FALSE;
}
...
int64_t compression(struct ssd_info *ssd)
{
    ...
    while(duration < COMP_DURATION)
    {
        ...
        if(cur_page == ssd->parameter->page_block)
        {
            ...
            if(!cur_ref_node || cur_ref_node == p_plane->history->tail)
                cur_ref_node = p_plane->history->head;
            else
