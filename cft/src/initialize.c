...
struct dram_info * initialize_dram(struct ssd_info * ssd)
{
    ...
    unsigned int active_lru_size = (page_num /(float)100);
    unsigned int inactive_lru_size = 2 * (page_num /(float)100);
    //adhoc1
    ssd->off_prom_thr = active_lru_size >> 1;
...
struct parameter_value *load_parameters(char parameter_file[30])
{
    ...
    while(fgets(buf,200,fp)){
        ...
        }else if((res_eql=strcmp(buf,"nameplate capacity")) ==0){
            sscanf(buf + next_eql,"%d",&p->nameplate_capacity_in_GB); 
        ...
        }else if((res_eql=strcmp(buf,"alpha")) ==0){
            sscanf(buf + next_eql,"%lf",&p->alpha); 
        }else if((res_eql=strcmp(buf,"min idle interval")) ==0){
            sscanf(buf + next_eql,"%d",&p->min_idle_interval); 
