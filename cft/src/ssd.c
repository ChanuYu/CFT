...
int get_requests(struct ssd_info *ssd, FILE *tracefile, int phase)  
{
    ...
    if (nearest_event_time==MAX_INT64)
    {
        if(phase!=PREFILL)
        {
            ...
            if(time_t - ssd->current_time > ssd->parameter->min_idle_interval)
            {
                ssd->current_time += ssd->parameter->min_idle_interval;
    ...
    while(req != NULL)	
    {
        ...
        if(req->response_time != 0)
        {
            ...
            if (req->operation==READ)
            {
                ...
                ssd->read_all_buffer_request_count++;
                ssd->read_avg=ssd->read_avg+(req->response_time-req->time);
            } 
            else
            {
                ssd->write_request_count++;
                ssd->write_all_buffer_request_count++;
            ...
            {		
                ...
                if (req->operation==READ)
                {
                    if(req->idle_trx_flag!=1)
                    {
                        ...
                        if(all_slc_flag)
                            ssd->read_all_slc_request_count++;
                    }
                    // {
                ...
                else
                {
                    if(req->idle_trx_flag!=1)
                    {
                        ...
                        if(all_slc_flag)
                            ssd->write_all_slc_request_count++;
                    }
                    else
                    {
                ...
                fprintf(outputfile,"%16lld %10d %6d %2d %16lld %16lld %10lld  D%d  SR%d  SW%d || %10lld\n",req->time,req->lsn, req->size, req->operation, start_time, end_time, end_time-req->time,req->decomp_read_count,req->slc_r_trx,req->slc_w_trx,avg_time);
                fflush(outputfile);
                // if(req->lsn>2) //lsn 0 or 1 => idle transaction
                // {
...
{
    unsigned int lpn_count=0,i,j,k,m,erase=0,plane_erase=0,c=0;
    ...
    for(i=0;i<ssd->parameter->channel_number;i++)
    {
        for(c=0;c<ssd->parameter->chip_channel[i];c++)
        {
            for(j=0;j<ssd->parameter->die_chip;j++)
            {
                for(k=0;k<ssd->parameter->plane_die;k++)
    ...
    fprintf(ssd->outputfile,"WAF W_host  (host write NAND pages):        %13lu\n",ssd->waf_w_host);
    fprintf(ssd->outputfile,"WAF W_write (basic write path NAND pages):  %13lu\n",ssd->waf_w_write);
    fprintf(ssd->outputfile,"WAF W_comp  (compression chunk writes):     %13lu\n",ssd->waf_w_comp);
    fprintf(ssd->outputfile,"WAF W_dem   (SLC GC demotion writes):       %13lu\n",ssd->waf_w_dem);
    fprintf(ssd->outputfile,"WAF W_prom  (promotion writes):             %13lu\n",ssd->waf_w_prom);
    if (ssd->waf_w_host > 0)
        fprintf(ssd->outputfile,"WAF W_total = (W_write+W_comp+W_dem+W_prom)/W_host: %.4f\n",
            (double)(ssd->waf_w_write + ssd->waf_w_comp + ssd->waf_w_dem + ssd->waf_w_prom) / ssd->waf_w_host);
    else
        fprintf(ssd->outputfile,"WAF W_total: N/A (W_host == 0)\n");
    fprintf(ssd->outputfile,"----------------------------------------\n");
    ...
    fprintf(ssd->outputfile,"all buffer request ratio to total request: %.3f\n",(ssd->read_all_buffer_request_count+ssd->write_all_buffer_request_count)/(float)(ssd->read_request_count+ssd->write_request_count));
    fprintf(ssd->outputfile,"all SLC request ratio to total request: %.3f\n",(ssd->read_all_slc_request_count+ssd->write_all_slc_request_count)/(float)(ssd->read_request_count+ssd->write_request_count));
    if(ssd->read_request_count > 0)
    {
        fprintf(ssd->outputfile,"read all buffer request ratio to read request: %.3f\n",(ssd->read_all_buffer_request_count)/(float)(ssd->read_request_count));
        fprintf(ssd->outputfile,"read all SLC request ratio: %.3f\n",(ssd->read_all_slc_request_count)/(float)(ssd->read_request_count+ssd->write_request_count));
        fprintf(ssd->outputfile,"read request average response time: %lld\n",ssd->read_avg/ssd->read_request_count);
    }
        
    if(ssd->write_request_count > 0)
    {
        fprintf(ssd->outputfile,"write all buffer request ratio to write request: %.3f\n",(ssd->write_all_buffer_request_count)/(float)(ssd->write_request_count));
        fprintf(ssd->outputfile,"write all SLC request ratio to write request: %.3f\n",(ssd->write_all_slc_request_count)/(float)(ssd->write_request_count));
    ...
    fprintf(ssd->outputfile,"qlc read count: %13d\n",ssd->qlc_r_trx);
    fprintf(ssd->outputfile,"slc write count: %13d\n",ssd->slc_w_trx);
    fprintf(ssd->outputfile,"qlc write count: %13d\n",ssd->qlc_w_trx);
    
    if(ssd->total_r_trx)
    {
        fprintf(ssd->outputfile,"slc read ratio to total read trx: %.2f\n",(ssd->slc_r_trx / (float)ssd->total_r_trx));
        fprintf(ssd->outputfile,"qlc read ratio to total read trx: %.2f\n",(ssd->qlc_r_trx / (float)ssd->total_r_trx));
        fprintf(ssd->outputfile,"Normalized slc read to qlc read: %.3f\n",ssd->slc_r_trx/(float)ssd->qlc_r_trx);
    }
    if(ssd->total_w_trx)
    {
        fprintf(ssd->outputfile,"slc write ratio to total write trx: %.2f\n",(ssd->slc_w_trx / (float)ssd->total_w_trx));
        fprintf(ssd->outputfile,"qlc write ratio to total write trx: %.2f\n",(ssd->qlc_w_trx / (float)ssd->total_w_trx));
        fprintf(ssd->outputfile,"Normalized slc write to qlc write: %.3f\n",ssd->slc_w_trx/(float)ssd->qlc_w_trx);
    }
    fprintf(ssd->outputfile,"Created SLC cache block ratio to total blocks: %.3f\n",ssd->total_slc_blk/(float)ssd->total_blocks);
    double created_slc_size = (ssd->total_slc_blk * ssd->parameter->page_capacity * (double)(ssd->parameter->page_block>>2))/((double)1000*1000*1000);
    fprintf(ssd->outputfile,"Created SLC cache size: %.3f GB\n",created_slc_size);
    double slc_cache_ratio_to_nameplate =  created_slc_size/(double)ssd->parameter->nameplate_capacity_in_GB;
    fprintf(ssd->outputfile,"SLC cache ratio to nameplate capacity: %.3f\n",slc_cache_ratio_to_nameplate);
    ...
    char output[512] = {0,};
    sprintf(output,"total used slc blk: %d, total slc blk: %d, total free slc blk: %d\n",total_used_slc_blk,ssd->total_slc_blk, ssd->total_free_slc_blk);
    fprintf(ssd->outputfile,"%s",output);
    printf("%s",output);

    fprintf(ssd->outputfile,"\ncompression failure ratio\n");
...

    ...
    fprintf(ssd->statisticfile,"total sub request count: %13d\n",ssd->total_trx_count);
    ...
    fprintf(ssd->statisticfile,"changed to QLC trx: %13d\n",ssd->changed_to_qlc_trx);

    fprintf(ssd->statisticfile,"\n--- WAF (Write Amplification Factor) ---\n");
    fprintf(ssd->statisticfile,"WAF W_host  (host write NAND pages):        %13lu\n",ssd->waf_w_host);
    fprintf(ssd->statisticfile,"WAF W_write (basic write path NAND pages):  %13lu\n",ssd->waf_w_write);
    fprintf(ssd->statisticfile,"WAF W_comp  (compression chunk writes):     %13lu\n",ssd->waf_w_comp);
    fprintf(ssd->statisticfile,"WAF W_dem   (SLC GC demotion writes):       %13lu\n",ssd->waf_w_dem);
    fprintf(ssd->statisticfile,"WAF W_prom  (promotion writes):             %13lu\n",ssd->waf_w_prom);
    if (ssd->waf_w_host > 0)
        fprintf(ssd->statisticfile,"WAF W_total = (W_write+W_comp+W_dem+W_prom)/W_host: %.4f\n",
            (double)(ssd->waf_w_write + ssd->waf_w_comp + ssd->waf_w_dem + ssd->waf_w_prom) / ssd->waf_w_host);
    else
        fprintf(ssd->statisticfile,"WAF W_total: N/A (W_host == 0)\n");
    fprintf(ssd->statisticfile,"----------------------------------------\n");
    ...
    fprintf(ssd->statisticfile,"all buffer request ratio to total request: %.3f\n",(ssd->read_all_buffer_request_count+ssd->write_all_buffer_request_count)/(float)(ssd->read_request_count+ssd->write_request_count));
    fprintf(ssd->statisticfile,"all SLC request ratio to total request: %.3f\n",(ssd->read_all_slc_request_count+ssd->write_all_slc_request_count)/(float)(ssd->read_request_count+ssd->write_request_count));
    if(ssd->read_request_count > 0)
    {
        fprintf(ssd->statisticfile,"read all buffer request ratio to read request: %.3f\n",(ssd->read_all_buffer_request_count)/(float)(ssd->read_request_count));
        fprintf(ssd->statisticfile,"read all SLC request ratio: %.3f\n",(ssd->read_all_slc_request_count)/(float)(ssd->read_request_count+ssd->write_request_count));
        fprintf(ssd->statisticfile,"read request average response time: %lld\n",ssd->read_avg/ssd->read_request_count);
    }
        
    if(ssd->write_request_count > 0)
    {
        fprintf(ssd->statisticfile,"write all buffer request ratio to write request: %.3f\n",(ssd->write_all_buffer_request_count)/(float)(ssd->write_request_count));
        fprintf(ssd->statisticfile,"write all SLC request ratio to write request: %.3f\n",(ssd->write_all_slc_request_count)/(float)(ssd->write_request_count));
        fprintf(ssd->statisticfile,"write request average response time: %lld\n",ssd->write_avg/ssd->write_request_count);
    }
    fprintf(ssd->statisticfile,"decomp read count: %13d\n",ssd->decomp_read_count);
    fprintf(ssd->statisticfile,"decomp read ratio to total read trx: %.2f\n",(ssd->decomp_read_count / (float)ssd->total_r_trx));
        
    fprintf(ssd->statisticfile,"slc read count: %13d\n",ssd->slc_r_trx);
    fprintf(ssd->statisticfile,"qlc read count: %13d\n",ssd->qlc_r_trx);
    fprintf(ssd->statisticfile,"total read count: %13d\n",ssd->total_r_trx);
    fprintf(ssd->statisticfile,"slc write count: %13d\n",ssd->slc_w_trx);
    fprintf(ssd->statisticfile,"qlc write count: %13d\n",ssd->qlc_w_trx);
    fprintf(ssd->statisticfile,"total write count: %13d\n",ssd->total_w_trx);
    
    if(ssd->total_r_trx)
    {
        fprintf(ssd->statisticfile,"slc read ratio to total read trx: %.2f\n",(ssd->slc_r_trx / (float)ssd->total_r_trx));
        fprintf(ssd->statisticfile,"qlc read ratio to total read trx: %.2f\n",(ssd->qlc_r_trx / (float)ssd->total_r_trx));
        fprintf(ssd->statisticfile,"Normalized slc read to qlc read: %.3f\n",ssd->slc_r_trx/(float)ssd->qlc_r_trx);
    }
    if(ssd->total_w_trx)
    {
        fprintf(ssd->statisticfile,"slc write ratio to total write trx: %.2f\n",(ssd->slc_w_trx / (float)ssd->total_w_trx));
        fprintf(ssd->statisticfile,"qlc write ratio to total write trx: %.2f\n",(ssd->qlc_w_trx / (float)ssd->total_w_trx));
        fprintf(ssd->statisticfile,"Normalized slc write to qlc write: %.3f\n",ssd->slc_w_trx/(float)ssd->qlc_w_trx);
    }
    fprintf(ssd->statisticfile,"Created SLC cache block ratio to total blocks: %.3f\n",ssd->total_slc_blk/(float)ssd->total_blocks);
    fprintf(ssd->statisticfile,"Created SLC cache size: %.3f GB\n",created_slc_size);
    fprintf(ssd->statisticfile,"SLC cache ratio to nameplate capacity: %.3f\n",slc_cache_ratio_to_nameplate);
    fprintf(ssd->statisticfile,"used_blocks / total_blocks: %.3f\n",ssd->used_blocks/(double)ssd->total_blocks);
    printf("used_blocks / total_blocks: %.3f\n",ssd->used_blocks/(double)ssd->total_blocks);
    printf("used blocks: %d, total blocks: %d\n",ssd->used_blocks, ssd->total_blocks);
    for(int i=0;i<ssd->parameter->channel_number;i++)
    {
        for(int j=0;j<ssd->parameter->chip_channel[i];j++)
        {
    ...
    sprintf(output,"total used slc blk: %d, total slc blk: %d, total free slc blk: %d\n",total_used_slc_blk,ssd->total_slc_blk, ssd->total_free_slc_blk);
    fprintf(ssd->statisticfile,"%s",output);

    fprintf(ssd->statisticfile,"\ncompression failure ratio\n");
    fprintf(ssd->statisticfile,"before COMP_START threshold: %f | blocked due to HIST_NODE_MIN_INTERVAL: %f | performance gain < performance cost: %f | step into compression: %f\n",one/(float)total,two/(float)total,three/(float)total,(1.0 - ((one+two+three)/(float)total)));
    fprintf(ssd->statisticfile,"before COMP_START threshold: %d | blocked due to HIST_NODE_MIN_INTERVAL: %d | performance gain < performance cost: %d | step into compression: %d\n",one,two,three,total - (one+two+three));
    fprintf(ssd->statisticfile,"idle count: %d\n",idle_count);
...
void process_lru_hash(struct ssd_info* ssd)
{
    ...
    for(unsigned int i=first_lpn;i<=last_lpn;i++)
    {
        ...
        if((res=get_hash_node(ssd->dram->active_hash,i)))
        {
            ...
            if(ssd->collect_offprom && new_request->operation==READ)
            {
