 ...
 *******************************************/
struct ssd_info *initiation(struct ssd_info *ssd, char * trace_name)
{
    ...
    //scanf("%s",trace_name);
    char dir[30] = "trace/";
    strcat(dir,trace_name);
    strcpy(ssd->tracefilename,dir);
    ...
    //scanf("%s",buf);
    //strcat(res,buf);
    strcat(res,trace_name);
    strcpy(ssd->outputfilename,res);
    //strncpy(ssd->outputfilename,"ex.out",7);
    strncpy(ssd->statisticfilename,"statistic10.dat",16);
    strncpy(ssd->latency_dist_file_name,"latency_dist.dat",17);

    strcat(res,"-prefill");
    printf("\n");
    ssd->prefill_outputfile=fopen(res,"w");
    if(ssd->prefill_outputfile==NULL)
    {
        printf("ssd->prefill_outputfile: the output file can't open %s\n",res);
        return NULL;
    }

    char ideal_dir[100] = "../ideal/slc_simul_time/";
    strcat(ideal_dir,trace_name);
    ssd->slc_simul_file = fopen(ideal_dir,"w");
    if(ssd->slc_simul_file==NULL)
    {
        printf("Cannot open slc_creation_time_file %s\n",ideal_dir);
        exit(1);
    }

    // char checkpoint[100] = {0,};
    // strcpy(checkpoint,res);
    // strcat(checkpoint,"_chk1.txt");

    // ssd->checkpoint1 = fopen(checkpoint,"w");
    // if(!ssd->checkpoint1)
    // {
    //     printf("Failed to open checkpoint1\n");
    //     exit(1);
    // }

    // int len = strlen(checkpoint);
    // checkpoint[len-5] = '2';
    // ssd->checkpoint2 = fopen(checkpoint,"w");
    // if(!ssd->checkpoint2)
    // {
    //     printf("Failed to open checkpoint1\n");
    //     exit(1);
    // }
    // checkpoint[len-5] = '3';
    // ssd->checkpoint3 = fopen(checkpoint,"w");
    // if(!ssd->checkpoint3)
    // {
    //     printf("Failed to open checkpoint1\n");
    //     exit(1);
    ...
    // checkpoint[len-5] = '4';
    // ssd->checkpoint4 = fopen(checkpoint,"w");
    // if(!ssd->checkpoint4)
    // {
    //     printf("Failed to open checkpoint1\n");
    //     exit(1);
    ...
    // checkpoint[len-5] = '5';
    // ssd->checkpoint5 = fopen(checkpoint,"w");
    // if(!ssd->checkpoint5)
    // {
    //     printf("Failed to open checkpoint1\n");
    //     exit(1);
    ...
    ssd->total_blocks = ssd->parameter->chip_num * ssd->parameter->die_chip * ssd->parameter->plane_die * ssd->parameter->block_plane;
    ...
    ssd->last_comp_loc.last_ref_plane = -1;
    ssd->gained_pages = 0;

    ssd->w_perform_gain_latency = (parameters->time_characteristics.tPROG_qlc - parameters->time_characteristics.tPROG_slc) / 1000;
    ssd->r_perform_gain_latency = (parameters->time_characteristics.tR_qlc - parameters->time_characteristics.tR_slc) / 1000;

    ssd->total_planes = ssd->parameter->plane_die * ssd->parameter->die_chip * ssd->parameter->chip_num;
    ...
    ssd->slc_cache_table[0] = 0.56f;
    ssd->slc_cache_table[1] = 0.5f;
    ssd->slc_cache_table[2] = 0.40f;
    ssd->slc_cache_table[3] = 0.30f;
    ssd->slc_cache_table[4] = 0.25f;
    ssd->slc_cache_table[5] = 0.20f;
    ssd->slc_cache_table[6] = 0.10f;
    init_dwa(ssd);
    ...
    {
        printf("ssd->outputfile: the output file can't open %s\n",ssd->outputfilename);
        return NULL;
    }
    ...
    ssd->latency_dist_file=fopen(ssd->latency_dist_file_name,"w");
    if(ssd->latency_dist_file==NULL)
    {
        printf("the latency_dist_file can't open\n");
        return NULL;
    }
...
struct dram_info * initialize_dram(struct ssd_info * ssd)
{
    ...
    //adhoc1
    ssd->off_prom_thr = page_num / (100 * 2);
    ...
    dram->active_hash = (struct hash_table*)malloc(sizeof(struct hash_table));
    init_hash_table(dram->active_hash, ACTIVE, page_num);
    dram->inactive_hash = (struct hash_table*)malloc(sizeof(struct hash_table));
    init_hash_table(dram->inactive_hash,INACTIVE,page_num);

    dram->active_lru = (struct hotdata_lru*)malloc(sizeof(struct hotdata_lru));
    init_lru(dram->active_lru,ACTIVE, ACTIVE_LRU_SIZE,dram->active_hash);
    dram->inactive_lru = (struct hotdata_lru*)malloc(sizeof(struct hotdata_lru));
    init_lru(dram->inactive_lru,INACTIVE, INACTIVE_LRU_SIZE,dram->inactive_hash);

    dram->comp_table = (struct comp_ratio_info*)malloc(sizeof(struct comp_ratio_info) * page_num);
    memset(dram->comp_table,0,sizeof(struct comp_ratio_info)*page_num);

    dram->comp_pool = (struct comp_pool*)malloc(sizeof(struct comp_pool));
    init_comp_pool(dram->comp_pool);

    dram->cht = (struct comp_hash_table*)malloc(sizeof(struct comp_hash_table));
    init_comp_h_table(dram->cht,page_num);
...
struct blk_info * initialize_block(struct blk_info * p_block,struct parameter_value *parameter, int blk_num)
{
    ...
    p_block->blk_num = blk_num;
    p_block->flash_mode = QLC;
    p_block->free_block_flag = FREE_BLK;
...
struct plane_info * initialize_plane(struct plane_info * p_plane,struct parameter_value *parameter )
{
    ...
    p_plane->qlc_active_blk = 0;
    p_plane->slc_active_blk = parameter->block_plane;
    ...
    for(i = 0; i<parameter->block_plane; i++)
    {
        ...
        initialize_block( p_block ,parameter,i);			
    }

    p_plane->history = (struct blk_history*)malloc(parameter->block_plane * sizeof(struct blk_history));
    init_blk_history(p_plane->history);

    p_plane->history_bm = (int*)malloc(parameter->block_plane * sizeof(int));
    for(i = 0; i<parameter->block_plane; i++)
        p_plane->history_bm[i] = BM_NONE;
    
    p_plane->slc_pool = (struct slc_pool*)malloc(sizeof(struct slc_pool));
    init_slc_pool(p_plane->slc_pool);
...
struct parameter_value *load_parameters(char parameter_file[30])
{
    ...
    while(fgets(buf,200,fp)){
        ...
        if((res_eql=strcmp(buf,"chip number")) ==0){			
        ...
        }else if((res_eql=strcmp(buf,"t_PROG_slc")) ==0){
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tPROG_slc); 
        }else if((res_eql=strcmp(buf,"t_PROG_qlc")) ==0){
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tPROG_qlc); 
        }else if((res_eql=strcmp(buf,"t_DBSY")) ==0){
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tDBSY); 
        }else if((res_eql=strcmp(buf,"t_BERS_slc")) ==0){
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tBERS_slc); 
        }else if((res_eql=strcmp(buf,"t_BERS_qlc")) ==0){
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tBERS_qlc); 
        ...
        }else if((res_eql=strcmp(buf,"t_R_slc")) ==0){
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tR_slc); 
        }else if((res_eql=strcmp(buf,"t_R_qlc")) ==0){
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tR_qlc); 
            ...
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tComp); 
        }else if((res_eql=strcmp(buf,"t_DECOMP")) ==0){
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tDecomp); 
        }else if((res_eql=strcmp(buf,"t_PRED")) ==0){
            sscanf(buf + next_eql,"%d",&p->time_characteristics.tPred); 
...
void init_slc_pool(struct slc_pool* pool)
{
    ...
    pool->tail = NULL;
    pool->num_blks = 0;
    pool->num_free_blks = 0;
}

void insert_to_slc_pool(struct slc_pool* pool, struct blk_info *blk)
{
    struct slc_blk_node *new = (struct slc_blk_node*)malloc(sizeof(struct slc_blk_node));
    new->blk = blk;
    new->next = NULL;
...
{
    struct slc_blk_node *prev = NULL, *curr = pool->head;
...
void remove_from_slc_pool(struct slc_pool* pool, unsigned int blk_num)
{
    ...
    if(!cur)
    {
        printf("SLC Pool is empty!\n");
        exit(1);
    }

    while(cur!=NULL)
    {
        if(cur->blk->blk_num == blk_num)
        {
            if(cur==pool->head)
            {
                pool->head = cur->next;
    ...
    printf("There's no such a block in the slc pool\n");
    exit(1);
}
...
{
    //QLC
    //int invalidate_count = 2 * ssd->parameter->page_block / 3;
    int invalidate_count = 3 * ssd->parameter->page_block / 4;

    if(blk->free_block_flag!=FREE_BLK)
    {
        printf("The block is not a free block\n");
        exit(1);
    }

    plane->free_page -= invalidate_count;
    blk->free_page_num -= invalidate_count;
    blk->invalid_page_num += invalidate_count;
    blk->last_write_page = invalidate_count - 1;
    blk->flash_mode = SLC;

    for(int i=0;i<invalidate_count;i++)
    {
        blk->page_head[i].lpn=0;
        blk->page_head[i].free_state = 0;
        blk->page_head[i].valid_state = 0;
    }

    ssd->total_slc_blk++;
    ssd->total_free_slc_blk++;

    fprintf(ssd->slc_simul_file,"%lld %d %d\n",ssd->current_time,CREATED,ssd->total_slc_blk);
}

int get_slc_cache_table_idx(float space_ratio)
{
    int rem = (int)(space_ratio * 10);
    switch(rem)
    {
        case 0: case 1:
            return 0;
...
float get_current_space_util(struct ssd_info *ssd)
{
    return ssd->used_blocks / (float)ssd->total_blocks;
}

float get_slc_ratio(struct ssd_info *ssd)
{
    if(ssd->total_slc_blk > 0)
        return 1.0f - (ssd->total_free_slc_blk / (float)ssd->total_slc_blk);
    else
        return 0.0f;
}

void adjust_slc_area(struct ssd_info *ssd)
{
    int cur_slc_table_idx = get_slc_cache_table_idx(get_current_space_util(ssd));

    // {
    //     incr_slc_area(ssd,cur_slc_table_idx);
    //     ssd->prev_slc_table_idx = cur_slc_table_idx;
    // }
    //else if(cur_slc_table_idx > ssd->prev_slc_table_idx)
    if(cur_slc_table_idx > ssd->prev_slc_table_idx)
    {
        decr_slc_area(ssd,cur_slc_table_idx);
        ssd->prev_slc_table_idx = cur_slc_table_idx;
    }
}

void incr_slc_area(struct ssd_info *ssd, int cur_stable_idx)
{
    struct parameter_value *pmv = ssd->parameter;
    int total_plane = pmv->plane_die*pmv->die_chip*pmv->chip_num;
    int total_slc_block = (int)((pmv->block_plane*total_plane)*(ssd->slc_cache_table[cur_stable_idx]-ssd->slc_cache_table[ssd->prev_slc_table_idx]));
    int slc_blk_plane = total_slc_block / total_plane;

    ssd->dwa_slc_blks += slc_blk_plane;
            ...
            for(int die=0;die<ssd->parameter->die_chip;die++){
                for(int plane=0;plane<ssd->parameter->plane_die;plane++){
                    ...
                    for(int block=0;block<ssd->parameter->block_plane;block++){
                        struct blk_info *blk = &(p_plane->blk_head[block]);
                        if(blk->free_block_flag!=FREE_BLK || p_plane->active_block==block)
                            continue;
                        make_slc_blk(ssd,p_plane,&(p_plane->blk_head[block]));
                        if(p_plane->slc_active_blk==ssd->parameter->block_plane)
                            p_plane->slc_active_blk = block;
                        insert_to_slc_pool(p_plane->slc_pool,blk);
                        
                        count++;
                        if(count==slc_blk_plane)
                            break;
                    }
                    cur_plane++;
                }
...
void decr_slc_area(struct ssd_info *ssd, int cur_stable_idx)
{
    struct parameter_value *pmv = ssd->parameter;
    int total_plane = pmv->plane_die*pmv->die_chip*pmv->chip_num;
    int total_slc_block = (int)((pmv->block_plane*total_plane)*(ssd->slc_cache_table[cur_stable_idx]-ssd->slc_cache_table[ssd->prev_slc_table_idx]));
    ...
    ssd->dwa_slc_blks -= slc_blk_plane;
    ssd->migration_count += slc_blk_plane * total_plane;

    for(int channel=0;channel<ssd->parameter->channel_number;channel++){
        for(int chip=0;chip<ssd->parameter->chip_channel[channel];chip++){
            for(int die=0;die<ssd->parameter->die_chip;die++){
                for(int plane=0;plane<ssd->parameter->plane_die;plane++){
                    for(int count=0;count<slc_blk_plane;count++)
                    {
                        struct plane_info *p_plane = &(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane]);
                        struct gc_operation *gc_node=(struct gc_operation *)malloc(sizeof(struct gc_operation));
                        alloc_assert(gc_node,"gc_node");
                        memset(gc_node,0, sizeof(struct gc_operation));

                        gc_node->next_node=NULL;
                        gc_node->migration_flag = 1;
                        gc_node->chip=chip;
                        gc_node->die=die;
                        gc_node->plane=plane;
...
void init_dwa(struct ssd_info *ssd)
{
    ssd->prev_slc_table_idx = 0;

    struct parameter_value *pmv = ssd->parameter;
    int total_plane = pmv->plane_die*pmv->die_chip*pmv->chip_num;
    int total_slc_block = (int)((pmv->block_plane*total_plane)*ssd->slc_cache_table[ssd->prev_slc_table_idx]);
    int slc_blk_plane = total_slc_block / total_plane;
