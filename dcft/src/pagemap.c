...
void add_slc_erase_node_in_idle(struct ssd_info *ssd,int channel)
{
    int i;
    int chip = (ssd->write_request_count+ssd->read_request_count)%ssd->parameter->chip_channel[channel];
    for(int a=0;a<ssd->parameter->chip_channel[channel];a++){
        //chip loop
        int die = 0;
        for(int b=0;b<ssd->parameter->die_chip;b++){
            //die loop
            int plane = (ssd->write_request_count+ssd->read_request_count)%ssd->parameter->plane_die;
            for(int c=0;c<ssd->parameter->plane_die;c++){
                //plane loop
                struct plane_info *p_plane = &(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane]);
                struct slc_pool *pool = p_plane->slc_pool;
                if(pool->num_free_blks>0)
                {
                    plane = (plane+1)%ssd->parameter->plane_die;
                    continue;
                }
                struct slc_blk_node *victim = pool->head;
                unsigned int qlc_active_block=p_plane->qlc_active_blk;
                unsigned int slc_active_block = get_slc_active_blk(ssd,channel,chip,die,plane,0);
                unsigned int invalid_page=0;
                int block = -1;
                while(victim!=NULL)
                {
                    i = victim->blk->blk_num;
                ...
                if(block != -1)
                {
                    add_slc_erase_node(ssd,channel,chip,die,plane,block);
                    return;
                }
                
                plane = (plane+1)%ssd->parameter->plane_die;
            }
            die = (die+1)%ssd->parameter->die_chip;
        }
...
{   
    ...
    ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].invalid_page_num++;
    memset(old_page->lpns,0,sizeof(unsigned int)*4);
    memset(old_page->valid_states,0,sizeof(int)*4);
    memset(old_page->free_states,0,sizeof(int)*4);
...
int uninterrupt_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane, unsigned int migration_flag)       
{
...
Status gc_for_channel(struct ssd_info *ssd, unsigned int channel)
{
    ...
    if (gc_node->priority==GC_UNINTERRUPT)
    {
        ...
        {
            flag_gc=uninterrupt_gc(ssd,channel,chip,die,plane,gc_node->migration_flag);
            if (flag_gc==1)
            {
