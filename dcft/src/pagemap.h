...
int move_page(struct ssd_info * ssd, struct local *location,unsigned int * transfer_size, int slc_gc_flag);
...
int update_cur_usage(struct ssd_info* ssd);
...
void add_slc_erase_node_to_chip(struct ssd_info *ssd, unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane, unsigned int block);
...
void delete_slc_gc_node_for_chip(struct ssd_info *ssd, struct chip_info *p_chip, struct slc_erase *erase_node);
